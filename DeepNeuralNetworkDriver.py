...
class DnnDriver:
    ...
    def __init__(self, model_name='DNN_Model', model_dir='Models', log_dir='log', run_dir='Snake', learning_rate=1e-3,
        ...
        self.layer_types = ['conv_2d', 'max_pool_2d', 'fully_connected']  # conv_2d, fully_connected, conv_1d
        self.layer_num_of_elements = [32, [2, 2], 20]  # number of layer elements
        self.layer_filter_sizes = [8, None, None]  # only for conv layers
        self.layer_strides = [1, [2, 2], None]  # only for conv layers
        ...
        for i in range(0, len(self.layer_types)):
            if self.layer_types[i] == 'fully_connected':
                ...
                self.network = conv_1d(self.network,
                                       ...
                                       activation=self.activations[i],
                                       weights_init=tnorm)
            elif self.layer_types[i] == 'max_pool_2d':
    ...
    def print_tensorboard_plots(self, plot_scalars=True, plot_histograms=False):
        tmp = os.path.join(self.log_dir, self.run_dir)
        try:
            log_path = os.path.join(tmp, os.listdir(tmp)[0])
            print('Loading log: ' + log_path)
        except FileNotFoundError:
            print('No log data in dir: ' + tmp)
            return
        pdf = PdfPages(os.path.join(self.model_dir, self.run_dir, "TrainingPlots.pdf"))
        start_time = time.time()
        reader = SummaryReader(log_path)
        print(time.time() - start_time)
        if plot_scalars:
            df_scalars = reader.scalars
            self.plot_scalars(pdf, df_scalars)
            del df_scalars
        if plot_histograms:
            df_histograms = reader.histograms
