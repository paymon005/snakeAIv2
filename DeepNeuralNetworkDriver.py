...
class DnnDriver:
    ...
    def train_model(self):
        ...
        if self.profile_run:
            ...
            )  # setup profiler so we can never use it
            tf.profiler.experimental.start(self.log_dir, options=options)
        step_snapshot_interval = self.snapshot_frequency * int(
            len(self.training_data) / 64
        )  # calculate save interval entirely wrong
        print(
            "Fitting the model and snapshotting every "
            + str(self.snapshot_frequency)
            + " epochs or every "
            + str(step_snapshot_interval)
            + " steps."
        )  # tell user save interval rate
        time.sleep(5)
        early_stopping_cb = EarlyStoppingCallback(
            accuracy_threshold=self.stop_accuracy
        )  # define non-working stop criteria, not that it will ever matter
        self.model.fit(
            {"input": X},
            {"targets": y},
            n_epoch=self.epochs,
            ...
            show_metric=True,
            run_id=self.run_id,
            snapshot_step=step_snapshot_interval,
    ...
    def neural_network_model(self):
        ...
        if (
            ...
            self.network = input_data(
                shape=[None, self.game_size[1], self.game_size[0], 1], name="input"
            )
        self.add_hidden_layers()  # add all the neural networks hidden layers
        self.network = fully_connected(
            self.network, self.output_size, activation=self.output_layer_activation
        )  # add a fully connected layer at the end to give the output (the output layer)
        self.network = regression(
            ...
            name="targets",
        )  # add a regression method, so this shit even works at all
        ...
        if self.save_checkpoints and self.model_dir is not None:  # setup save path
            ...
            best_checkpoint_dir = os.path.join(
                self.model_dir, self.run_dir, "best-" + self.model_name
            )
            self.model = tflearn.DNN(
                self.network,
                tensorboard_dir=self.log_dir,
            ...
            self.model = tflearn.DNN(
                self.network, tensorboard_dir=self.log_dir
            )  # initialize neural network
        return self.model
    ...
    def get_last_run(self):
        ...
        files = sorted(
            files, key=os.path.getctime, reverse=True
        ...
        for file in files:
            if file.endswith(".meta"):  # if its model file, save the name
                ...
                tmp = model_to_load.split(
                    "\\"
                )  # get rid of the top folder from the path
                run_dir = "\\".join(
                    tmp[1:-1]
                )  # get rid of the top folder from the path
                break
    ...
    def plot_graphs(self):
        os.system(
            "taskkill /IM " "tensorboard.main" " /F"
        )  # one of these works, I dunno
        os.system(
            "taskkill /IM " "tensorboard.exe" " /F"
        )  # one of these works, I dunno
        print(
            os.getcwd()
            + "\\venv\\Scripts\\python.exe -m tensorboard.main --logdir="
            + self.log_dir
        ...
        os.system(
            ...
            + self.log_dir
            + "--samples_per_plugin scalars=9999999999,images=9999999999"
        )  # start tensorboard without trimming data cause I have the RAM to do it
    ...
    def load_training_data(self, filename=None):
        if filename is not None:
            file_to_load = MyTools.find_file_in_folder(
                self.model_dir, filename
            )  # load specific training data
        else:
            file_to_load = MyTools.get_newest_file_in_folder_w_ext(
                self.model_dir, "npy"
            )  # load newest training data
        print("Loading: " + file_to_load)
        np_load_old = np.load  # some stack overflow np funk
        np.load = lambda *a, **k: np_load_old(
            *a, allow_pickle=True, **k
        )  # some stack overflow np funk
    ...
    def save_layers(
        self,
    ):  # save layers so when we load the model we can make a new matching one, if its been changed
        data = {}
        file = open(
            os.path.join(self.model_dir, self.run_dir, "Model_Network.pkl"), "wb"
        )
    ...
    def load_layers(
        self,
    ):  # load layers so we can overwrite the ones defined so the new DNN can be made, so we can process game data and feed it to the model correctly
        ...
        if os.path.isfile(path):
            try:
                ...
                raise Exception(
                    "Outdated model version, does not contain required data for network generation!"
                )
        else:
    ...
    def print_tensorboard_plots(
        self, plot_scalars=True, plot_histograms=False
    ):  # print tensorboard plots data to PDF
        ...
        try:
            ...
            print(
                "No log data in dir: " + tmp
            )  # catch the error of just not finding shit
            return
        pdf = PdfPages(
            os.path.join(self.model_dir, self.run_dir, "TrainingPlots.pdf")
        )  # make pdf to save plots
...
class EarlyStoppingCallback(
    callbacks.Callback
):  # stop the model fitting if its already accurate
